...
class Renderer {
 public:
  /**
   * @brief Which graphics API the Renderer should select for use at
   * initialization
   */
  ...
  enum RendererFeature : int32_t {
    ...
    kFeature_DisjointTimerQuery = 1,  ///< Does the device support disjoint
                                      ///< timer query needed for GPU PerfHint
  };
  ...
  /**
   * @brief Set the graphics API that the renderer should use. This should be
   * called prior to calling ::GetInstance for the first time.
   ...
   */
  static base_game_framework::DisplayManager::SwapchainHandle
  GetSwapchainHandle() {
    return Renderer::swapchain_handle_;
  }

  /**
   * @brief Set a handle to the Display Manager swapchain  that the renderer
   * should use. This should be called prior to calling ::GetInstance for the
   * first time.
  ...
  /**
   * @brief Retrieve an instance of the renderer interface. The first time this
   * is called it will construct and initialize the renderer. Before calling for
   * the first time you must call Renderer::SetRendererAPI to set the graphics
   * API to use.
   * @return Reference to the `Renderer` interface class.
   */
  static Renderer& GetInstance();
  /**
   * @brief Shuts down the renderer and frees all graphical resources in the
  ...
  /**
   * @brief Determine if a feature is available for the current API on the
  ...
  virtual void SetupQueryTimer() = 0;
  virtual void StartQueryTimer() = 0;
  virtual void EndQueryTimer() = 0;

  /**
   * @brief Tell the renderer to set up to begin rendering a frame of draw
   * calls.
   */
  ...
  /**
   * @brief Draw a sequence of vertices using bound resources and the current
   * render state.
   * @param vertex_count Number of vertices to draw from the bound vertex
   * buffer.
   * @param first_vertex Vertex offset into the bound vertex buffer to begin
   * drawing from.
   */
  virtual void Draw(const uint32_t vertex_count,
                    const uint32_t first_vertex) = 0;
  /**
   * @brief Draw a sequence of indexed vertices using bound resources and the
   * curent render state.
   * @param index_count Number of indices to draw from the bound index buffer.
   * @param first_index Index offset into the bound index buffer to begin
   * drawing from.
   */
  ...
  /**
   * @brief Set a render pass as the current one for rendering. Binds the
   * drawable resources associated with the render pass.
  ...
  /**
   * @brief Set the current render state to use for rendering. Binds resources
   * associated with the render state. Note that the `RenderPass` object
   * associated with the `RenderState` being used must have previously been set
   * as current using the ::SetRenderPass function.
  ...
  virtual void BindVertexBuffer(
      std::shared_ptr<VertexBuffer> vertex_buffer) = 0;
  /**
   ...
   * @param params A reference to a `IndexBufferCreationParams` struct with
   * creation parameters.
   ...
   * @param index_buffer A shared pointer to a renderer `IndexBuffer`. Do not
   * retain any other instances of the shared pointer after calling the destroy
   * function. The resources is not immediately deleted, but put in a delete
   * queue. Deletion will happen at the next ::BeginFrame or ::ShutdownInstance.
   */
  virtual void DestroyIndexBuffer(
      std::shared_ptr<IndexBuffer> index_buffer) = 0;

  /**
   * @brief Create a renderer `RenderPass`.
   * @param params A reference to a `RenderPassCreationParams` struct with
   * creation parameters.
   ...
   * @param render_pass A shared pointer to a renderer `RenderPass`. Do not
   * retain any other instances of the shared pointer after calling the destroy
   * function. The resources is not immediately deleted, but put in a delete
   * queue. Deletion will happen at the next ::BeginFrame or ::ShutdownInstance.
   */
   ...
   * @param params A reference to a `RenderStateCreationParams` struct with
   * creation parameters.
   ...
   * @param render_state A shared pointer to a renderer `RenderState`. Do not
   * retain any other instances of the shared pointer after calling the destroy
   * function. The resources is not immediately deleted, but put in a delete
   * queue. Deletion will happen at the next ::BeginFrame or ::ShutdownInstance.
   */
  virtual void DestroyRenderState(
      std::shared_ptr<RenderState> render_state) = 0;

  /**
   * @brief Create a renderer `ShaderProgram`.
   * @param params A reference to a `ShaderProgramCreationParams` struct with
   * creation parameters.
   ...
   * @param shader_program A shared pointer to a renderer `ShaderProgram`. Do
   * not retain any other instances of the shared pointer after calling the
   * destroy function. The resources is not immediately deleted, but put in a
   * delete queue. Deletion will happen at the next ::BeginFrame or
   * ::ShutdownInstance.
   */
  virtual void DestroyShaderProgram(
      std::shared_ptr<ShaderProgram> shader_program) = 0;

  /**
   * @brief Create a renderer `Texture`.
   * @param params A reference to a `TextureCreationParams` struct with creation
   * parameters.
   ...
   * @param texture A shared pointer to a renderer `Texture`. Do not retain any
   * other instances of the shared pointer after calling the destroy function.
   * The resources is not immediately deleted, but put in a delete queue.
   * Deletion will happen at the next ::BeginFrame or ::ShutdownInstance.
   */
  virtual void DestroyTexture(std::shared_ptr<Texture> texture) = 0;

  /**
   * @brief Create a renderer `UniformBuffer`.
   * @param params A reference to a `UniformBufferCreationParams` struct with
   * creation parameters.
   ...
   * @param uniform_buffer A shared pointer to a renderer `UniformBuffer`. Do
   * not retain any other instances of the shared pointer after calling the
   * destroy function. The resources is not immediately deleted, but put in a
   * delete queue. Deletion will happen at the next ::BeginFrame or
   * ::ShutdownInstance.
  ...
  virtual void DestroyUniformBuffer(
      std::shared_ptr<UniformBuffer> uniform_buffer) = 0;

  /**
   * @brief Create a renderer `VertexBuffer`.
   * @param params A reference to a `VertexBufferCreationParams` struct with
   * creation parameters.
   ...
   * @param vertex_buffer A shared pointer to a renderer `VertexBuffer`. Do not
   * retain any other instances of the shared pointer after calling the destroy
   * function. The resources is not immediately deleted, but put in a delete
   * queue. Deletion will happen at the next ::BeginFrame or ::ShutdownInstance.
   */
