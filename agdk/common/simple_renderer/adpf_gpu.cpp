...
AdpfGpu::~AdpfGpu() { uninitializePerformanceHintManager(); }

void AdpfGpu::initializePerformanceHintManager(int32_t *thread_ids,
                                               size_t thread_size,
  ...
  performance_hint_session_ = APerformanceHint_createSession(
      performance_hint_manager_, thread_ids, thread_size, target_work_duration);
...
void AdpfGpu::setGpuTimestampPeriod(float timestamp_period) {
  ALOGI("AdpfGpu::setGpuTimestampPeriod: %f", timestamp_period);
...
void AdpfGpu::setWorkPeriodStartTimestampNanos(int64_t cpu_timestamp) {
  if (performance_hint_manager_ != nullptr &&
...
#if __ANDROID_API__ >= 35
    ALOGI("AdpfGpu::setWorkPeriodStartTimestampNanos %" PRIu64 "",
          cpu_timestamp);
    ...
    ALOGI(
        "AdpfGpu::setWorkPeriodStartTimestampNanos performance_hint_manager_ = "
        "%p work_duration_ = %p",
...
void AdpfGpu::setActualCpuDurationNanos(int64_t cpu_duration) {
  if (performance_hint_manager_ != nullptr &&
    ...
    ALOGI(
        "AdpfGpu::setActualCpuDurationNanos performance_hint_manager_ = %p "
        "work_duration_ = %p",
        performance_hint_manager_, work_duration_);
  }
}
void AdpfGpu::setActualGpuDurationNanos(int64_t gpu_duration,
  ...
  if (performance_hint_manager_ != nullptr &&
    ...
    ALOGI(
        "AdpfGpu::setActualGpuDurationNanos gpu_timestamp_period_set: %d "
        "gpu_duration: %" PRId64
    ...
    ALOGI(
        "AdpfGpu::setActualGpuDurationNanos performance_hint_manager_ = %p "
        "work_duration_ = %p",
        performance_hint_manager_, work_duration_);
  }
}
void AdpfGpu::setActualTotalDurationNanos(int64_t cpu_duration) {
  if (performance_hint_manager_ != nullptr &&
    ...
    ALOGI(
        "AdpfGpu::setActualTotalDurationNanos performance_hint_manager_ = %p "
        "work_duration_ = %p",
        performance_hint_manager_, work_duration_);
  }
}

void AdpfGpu::updateTargetWorkDuration(int64_t target_work_duration) {
  // ALOGI("AdpfGpu::updateTargetWorkDuration %" PRId64 " == %" PRId64 "",
  // target_work_duration_, target_work_duration);
  if (performance_hint_manager_ != nullptr &&
      performance_hint_session_ != nullptr && work_duration_ != nullptr) {
#if __ANDROID_API__ >= 35
    if (target_work_duration_ != target_work_duration) {
      // ALOGI("AdpfGpu::updateTargetWorkDuration %" PRIu64 "",
      ...
      ALOGI("AdpfGpu::updateTargetWorkDuration %" PRIu64 " RESULT: %d",
            target_work_duration, result);
    }
#endif
  } else {
    ALOGI(
        "AdpfGpu::updateTargetWorkDuration performance_hint_manager_ = %p "
        "work_duration_ = %p",
        performance_hint_manager_, work_duration_);
  }
}
...
void AdpfGpu::reportActualWorkDuration() {
  if (performance_hint_manager_ != nullptr &&
      performance_hint_session_ != nullptr && work_duration_ != nullptr) {
#if __ANDROID_API__ >= 35
    // ALOGI("AdpfGpu::reportActualWorkDuration %" PRIu64 " %" PRIu64 " %"
    // PRIu64 " %" PRIu64 " ",
    ...
    ALOGI(
        "AdpfGpu::reportActualWorkDuration performance_hint_manager_ = %p "
        "work_duration_ = %p",
        performance_hint_manager_, work_duration_);
  }
}
...
//         AWorkDuration_setActualGpuDurationNanos(work_duration_,
//         work_duration);
//         //APerformanceHint_reportActualWorkDuration2(performance_hint_session_,
//         work_duration_);
// #endif
//     } else {
//         ALOGI("AdpfGpu::reportGpuWorkDuration performance_hint_manager_ is
//         null : %p", performance_hint_manager_);
//     }
// }
