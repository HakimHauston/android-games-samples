...
messengerCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
  ...
  DebugManager::Log(DebugManager::kLog_Channel_Default,
                    DebugManager::kLog_Level_Error, "Vulkan Validation",
...
void GraphicsAPIVulkan::QueryAvailability() {
  // There is a dependency on a native window being available to attach a
  // surface so we can't query for devices and their capabilities until the
...
void GraphicsAPIVulkan::QueryCapabilities() {
  use_physical_device_properties2_ =
      VulkanAPIUtils::GetUsePhysicalDeviceProperties2();
    ...
    if (vk_instance_ != VK_NULL_HANDLE) {
      ...
      if (vk_surface_ != VK_NULL_HANDLE) {
        ...
        if (vk_physical_device_ != VK_NULL_HANDLE) {
          ...
          VulkanAPIUtils::GetDisplayFormats(vk_physical_device_, vk_surface_,
                                            display_formats_);
          PlatformUtilVulkan::GetScreenResolutions(surface_capabilities_,
                                                   display_resolutions_);
          ...
          if (CreateDevice(true, queue_indices)) {
            ...
            if (!display_formats_.empty() && !display_resolutions_.empty()) {
              ...
              // swapchain_interval_ =
              // DisplayManager::kDisplay_Swap_Interval_60FPS; // FORCE_FPS:
              // bug? this is accidentally fixed to 60FPS for Vulkan
              if (CreateSwapchain(queue_indices)) {
                PlatformUtilVulkan::GetRefreshRates(
                    vk_physical_device_, vk_device_, vk_swapchain_,
                    vk_present_queue_, present_queue_index_, swap_intervals_);
                DestroySwapchain();
              }
            }
            DestroyDevice();
...
void GraphicsAPIVulkan::QueryDeviceCapabilities(
  ...
  if (!use_physical_device_properties2_) {
    // If VK_KHR_get_physical_device_properties2 isn't supported, assume a
    // really old 1.0.x device and early out.
    DebugManager::Log(DebugManager::kLog_Channel_Default,
  ...
  const bool has_driver_properties = VulkanAPIUtils::HasDeviceExtension(
      available_extensions, VK_KHR_DRIVER_PROPERTIES_EXTENSION_NAME);
  const bool has_shader_float16_int8 = VulkanAPIUtils::HasDeviceExtension(
      available_extensions, VK_KHR_SHADER_FLOAT16_INT8_EXTENSION_NAME);
  if (has_shader_float16_int8) {
    enabled_device_extensions_.push_back(
        VK_KHR_SHADER_FLOAT16_INT8_EXTENSION_NAME);
  }
  const bool has_16bit_storage = VulkanAPIUtils::HasDeviceExtension(
      available_extensions, VK_KHR_16BIT_STORAGE_EXTENSION_NAME);
  ...
  device_driver_properties.sType =
  ...
  // + GPU_PERF_HINT
  AdpfGpu::getInstance().setGpuTimestampPeriod(
  ...
  // Retrieve device features, including optional extensions on reduced
  // precision formats
...
void GraphicsAPIVulkan::QuerySurfaceCapabilities() {
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(vk_physical_device_, vk_surface_,
  ...
  for (const VkPresentModeKHR &mode : present_modes) {
    switch (mode) {
      // The DisplayManager enums map directly to the VkPresentMode values we
      // currently support
...
void GraphicsAPIVulkan::DetermineAPILevel(const uint32_t api_version,
  ...
  const uint32_t instance_major_version =
      VK_VERSION_MAJOR(instance_api_version);
  const uint32_t instance_minor_version =
  ...
  const uint32_t api_major_version =
      (instance_major_version < device_major_version) ? instance_major_version
                                                      : device_major_version;
  const uint32_t api_minor_version =
      (instance_minor_version < device_minor_version) ? instance_minor_version
                                                      : device_minor_version;

  DebugManager::Log(
      DebugManager::kLog_Channel_Default, DebugManager::kLog_Level_Info,
      BGM_CLASS_TAG,
      "VULKAN_VERSIONS api %u.%u instance %u.%u device %u.%u real %u.%u",
      VK_VERSION_MAJOR(api_version), VK_VERSION_MINOR(api_version),
      instance_major_version, instance_minor_version, device_major_version,
      device_minor_version, api_major_version, api_minor_version);

  if (api_version >= VK_API_VERSION_1_3 && api_major_version >= 1 &&
      api_minor_version >= 3) {
...
void GraphicsAPIVulkan::DetermineNumericSupport(
  ...
  if (shader_int16) {
    api_features_.SetGraphicsFeature(
        GraphicsAPIFeatures::kGraphicsFeature_I16_Math);
  }
  if (shader_float_16_int_8_features.shaderInt8) {
    api_features_.SetGraphicsFeature(
        GraphicsAPIFeatures::kGraphicsFeature_I8_Math);
  }
  if (shader_float_16_int_8_features.shaderFloat16) {
    api_features_.SetGraphicsFeature(
        GraphicsAPIFeatures::kGraphicsFeature_F16_Math);
  }
  if (device_16_bit_storage_features.storageBuffer16BitAccess) {
    api_features_.SetGraphicsFeature(
        GraphicsAPIFeatures::kGraphicsFeature_F16_I16_SSBO);
  }
  if (device_16_bit_storage_features.uniformAndStorageBuffer16BitAccess) {
    api_features_.SetGraphicsFeature(
        GraphicsAPIFeatures::kGraphicsFeature_F16_I16_SSBO);
    api_features_.SetGraphicsFeature(
        GraphicsAPIFeatures::kGraphicsFeature_F16_I16_UBO);
  }
  if (device_16_bit_storage_features.storagePushConstant16) {
    api_features_.SetGraphicsFeature(
        GraphicsAPIFeatures::kGraphicsFeature_F16_I16_Push_Constant);
  }
  if (device_16_bit_storage_features.storageInputOutput16) {
    api_features_.SetGraphicsFeature(
        GraphicsAPIFeatures::kGraphicsFeature_F16_I16_Input_Output);
  }
}

bool GraphicsAPIVulkan::DisplayFormatExists(
...
bool GraphicsAPIVulkan::InitializeGraphicsAPI() {
  if (!PlatformUtilVulkan::GetValidationLayersAvailable()) {
    ...
    DebugManager::Log(
        DebugManager::kLog_Channel_Default, DebugManager::kLog_Level_Error,
        BGM_CLASS_TAG,
    ...
    if (vk_surface_ != VK_NULL_HANDLE) {
      ...
      if (vk_physical_device_ != VK_NULL_HANDLE) {
        QueueFamilyIndices queue_indices =
            VulkanAPIUtils::FindQueueFamilies(vk_physical_device_, vk_surface_);
...
GraphicsAPIVulkan::GenerateSwapchainConfigurations() {
  DisplayManager::SwapchainConfigurations *swap_configs =
      new DisplayManager::SwapchainConfigurations(
          ...
          swapchain_min_frames_, swapchain_max_frames_,
          swapchain_present_modes_, DisplayManager::kDefault_Display);
  return swap_configs;
}
...
DisplayManager::InitSwapchainResult GraphicsAPIVulkan::InitSwapchain(
  ...
  for (const DisplayManager::DisplayResolution &iter_resolution :
       display_resolutions_) {
  ...
  for (const DisplayManager::DisplaySwapInterval &iter_interval :
       swap_intervals_) {
  ...
  if (!PlatformUtilVulkan::ActivateSwapchain(
          vk_physical_device_, vk_device_, vk_swapchain_, vk_present_queue_,
          present_queue_index_, swapchain_interval_)) {
    return DisplayManager::kInit_Swapchain_Failure;
  }
...
DisplayManager::SwapchainRotationMode
GraphicsAPIVulkan::GetSwapchainRotationMode() {
  DisplayManager::SwapchainRotationMode mode =
      DisplayManager::kSwapchain_Rotation_None;
...
GraphicsAPIVulkan::PresentCurrentSwapchainFrame() {
  if (swapchain_acquired_current_frame_image_) {
    ...
    VkResult result =
        PlatformUtilVulkan::PresentSwapchain(vk_present_queue_, &present_info);
    if (result == VK_SUBOPTIMAL_KHR) {
      DebugManager::Log(DebugManager::kLog_Channel_Default,
                        DebugManager::kLog_Level_Warning, BGM_CLASS_TAG,
                        "vkQueuePresentKHR returned VK_SUBOPTIMAL_KHR");
      // Update transform flags and recreate swapchain
      vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
          vk_physical_device_, vk_surface_, &surface_capabilities_);
      pretransform_flags_ = surface_capabilities_.currentTransform;
      RecreateSwapchain();
...
bool GraphicsAPIVulkan::GetGraphicsAPIResourcesVk(
    GraphicsAPIResourcesVk &api_resources_vk) {
...
bool GraphicsAPIVulkan::GetSwapchainFrameResourcesVk(
  ...
  if (frame_handle != DisplayManager::kInvalid_swapchain_handle &&
    ...
    if (!swapchain_acquired_current_frame_image_ && acquire_frame_image) {
      vkWaitForFences(
          vk_device_, 1,
          &in_flight_frame_fence_[swapchain_info_
                                      .swapchain_current_frame_index_],
          VK_TRUE, UINT64_MAX);

      VkResult result = vkAcquireNextImageKHR(
          vk_device_, vk_swapchain_, UINT64_MAX,
          swapchain_image_semaphore_[swapchain_info_
                                         .swapchain_current_frame_index_],
          VK_NULL_HANDLE, &swapchain_info_.swapchain_current_image_index_);
      if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        RecreateSwapchain();

        DebugManager::Log(
            DebugManager::kLog_Channel_Default,
            DebugManager::kLog_Level_Warning, BGM_CLASS_TAG,
    ...
    frame_resources.render_complete =
        frame_render_completion_semaphore_[swapchain_info_
                                               .swapchain_current_frame_index_];
    frame_resources.swapchain_color_image_view =
        swapchain_info_.swapchain_image_views_
            [swapchain_info_.swapchain_current_image_index_];
    ...
    frame_resources.swapchain_color_format =
    ...
    if (pretransform_flags_ & VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR ||
      ...
      frame_resources.swapchain_extent.width =
          swapchain_resolution_.display_height;
      frame_resources.swapchain_extent.height =
          swapchain_resolution_.display_width;
    } else {
      frame_resources.swapchain_extent.width =
          swapchain_resolution_.display_width;
      frame_resources.swapchain_extent.height =
    ...
    frame_resources.swapchain_frame_index =
        swapchain_info_.swapchain_current_frame_index_;
    frame_resources.swapchain_image_index =
        swapchain_info_.swapchain_current_image_index_;
    return true;
  }
...
bool GraphicsAPIVulkan::SetDisplayChangedCallback(
...
void GraphicsAPIVulkan::SwapchainChanged(
...
bool GraphicsAPIVulkan::CreateDevice(bool is_preflight_check,
  ...
  const bool enable_validation =
      !is_preflight_check && enable_validation_layers_;
  ...
  if (api_features_.HasGraphicsFeature(
          GraphicsAPIFeatures::kGraphicsFeature_I16_Math)) {
  ...
  VulkanAPIUtils::CheckVkResult(
      vkCreateDevice(vk_physical_device_, &device_create_info, nullptr,
                     &vk_device_),
      "vkCreateDevice");

  vkGetDeviceQueue(vk_device_, queue_indices.graphics_family.value(), 0,
                   &vk_graphics_queue_);
  vkGetDeviceQueue(vk_device_, queue_indices.present_family.value(), 0,
                   &vk_present_queue_);
  ...
  if (enable_validation) {
    ...
    vkCreateDebugUtilsMessengerEXT(vk_instance_, &messenger_create_info,
...
void GraphicsAPIVulkan::CreateInstance(bool is_preflight_check) {
  // Skip validation layers if we are just creating a preflight instance to
  // query device extensions and capabilities
  const bool request_validation =
      !is_preflight_check && enable_validation_layers_;

  const std::vector<const char *> instance_extensions =
      PlatformUtilVulkan::GetRequiredInstanceExtensions(
          request_validation, use_physical_device_properties2_);
  ...
  const VkResult result =
      vkCreateInstance(&create_info, nullptr, &vk_instance_);
...
bool GraphicsAPIVulkan::CreateSwapchain(
    const QueueFamilyIndices &queue_indices) {
  ...
  VulkanAPIUtils::CheckVkResult(
      vkCreateSwapchainKHR(vk_device_, &swapchain_create_info, nullptr,
                           &vk_swapchain_),
...
void GraphicsAPIVulkan::CreateSwapchainImages() {
  ...
  vkGetSwapchainImagesKHR(vk_device_, vk_swapchain_,
                          &swapchain_info_.swapchain_image_count_, nullptr);
  swapchain_info_.swapchain_images_.resize(
      swapchain_info_.swapchain_image_count_);
  vkGetSwapchainImagesKHR(vk_device_, vk_swapchain_,
                          &swapchain_info_.swapchain_image_count_,
                          swapchain_info_.swapchain_images_.data());

  swapchain_info_.swapchain_image_views_.resize(
      swapchain_info_.swapchain_image_count_);
  VkImageView *base_view = swapchain_info_.swapchain_image_views_.data();
  const VkFormat pixel_format =
      VulkanAPIUtils::GetVkPixelFormat(swapchain_format_.display_pixel_format);
  ...
  if (swapchain_info_.swapchain_depth_stencil_format_ != VK_FORMAT_UNDEFINED) {
    ...
    VkImage *swap_images =
        swapchain_info_.swapchain_depth_stencil_images_.data();
    VkImageView *swap_image_views =
        swapchain_info_.swapchain_depth_stencil_image_views_.data();
    VmaAllocation *swap_allocs =
        swapchain_info_.swapchain_depth_stencil_allocs_.data();
    ...
    for (size_t i = 0; i < swapchain_info_.swapchain_image_count_; ++i) {
      ...
      VulkanAPIUtils::CheckVkResult(
          vmaCreateImage(allocator_, &ds_image_info,
                         &ds_image_alloc_create_info, &swap_images[i],
                         &swap_allocs[i], nullptr),
          "vmaCreateImage swap depth/stencil");

      VkImageViewCreateInfo ds_image_view_info = {
          VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO};
      ds_image_view_info.image = swap_images[i];
      ds_image_view_info.viewType = VK_IMAGE_VIEW_TYPE_2D;
      ds_image_view_info.format =
          swapchain_info_.swapchain_depth_stencil_format_;
      ds_image_view_info.subresourceRange.aspectMask =
      ...
      VulkanAPIUtils::CheckVkResult(
          vkCreateImageView(vk_device_, &ds_image_view_info, nullptr,
                            &swap_image_views[i]),
...
void GraphicsAPIVulkan::DestroySwapchainImages() {
  ...
  if (!swapchain_info_.swapchain_depth_stencil_images_.empty()) {
    for (size_t i = 0;
         i < swapchain_info_.swapchain_depth_stencil_images_.size(); ++i) {
  ...
  if (!swapchain_info_.swapchain_image_views_.empty()) {
    for (size_t i = 0; i < swapchain_info_.swapchain_image_count_; ++i) {
      vkDestroyImageView(vk_device_, swapchain_info_.swapchain_image_views_[i],
                         nullptr);
    }
  }
...
void GraphicsAPIVulkan::CreateSynchronizationObjects() {
  const size_t in_flight_frame_count =
      DisplayManager::GetInstance().GetDisplayBufferMode();
  ...
  for (size_t i = 0; i < in_flight_frame_count; ++i) {
    VulkanAPIUtils::CheckVkResult(
        vkCreateSemaphore(vk_device_, &semaphore_info, nullptr,
    ...
    VulkanAPIUtils::CheckVkResult(
        vkCreateFence(vk_device_, &fence_info, nullptr,
...
void GraphicsAPIVulkan::DestroySynchronizationObjects() {
  for (size_t i = 0; i < in_flight_frame_fence_.size(); ++i) {
    vkDestroySemaphore(vk_device_, frame_render_completion_semaphore_[i],
                       nullptr);
