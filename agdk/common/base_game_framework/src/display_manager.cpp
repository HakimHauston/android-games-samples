...
static constexpr DisplayManager::SwapchainHandle kDefault_swapchain_handle =
    123;
...
static constexpr uint64_t kSwap_interval_constants[] = {
    ...
    DisplayManager::kDisplay_Swap_Interval_144FPS,
...
void DisplayManager::ShutdownInstance() { DisplayManager::instance_.reset(); }
  ...
  if (api == kGraphicsAPI_Vulkan) {
    switch (api_vulkan_->GetAPIStatus()) {
      case kGraphicsAPI_Uninitialized:
        break;
      case kGraphicsAPI_ObtainingAvailability:
        api_vulkan_->QueryAvailability();
        returnFlags = kGraphics_API_Waiting;
        break;
      case kGraphicsAPI_AvailabilityReady:
      case kGraphicsAPI_Active:
        returnFlags = api_vulkan_->GetFeatureFlags();
        break;
      default:
        break;
    }
...
DisplayManager::InitGraphicsAPIResult DisplayManager::InitGraphicsAPI(
    const GraphicsAPI api, const uint32_t requested_features) {
  ...
  DisplayManager::InitGraphicsAPIResult result =
      kInit_GraphicsAPI_Failure_Unsupported;
...
void DisplayManager::ShutdownGraphicsAPI() {
  if (active_api_ != kGraphicsAPI_None &&
      api_->GetAPIStatus() == kGraphicsAPI_Active) {
...
DisplayManager::DisplayId DisplayManager::GetDisplayId(
...
DisplayManager::InitSwapchainResult DisplayManager::InitSwapchain(
  ...
  if (display_id != kDefault_Display) {
    // Multiple display support is a TODO: require the default constant right
    // now
  ...
  if (active_api_ != kGraphicsAPI_None &&
    ...
    result = api_->InitSwapchain(display_format, display_resolution,
                                 display_swap_interval, swapchain_frame_count,
                                 present_mode);
...
DisplayManager::PresentCurrentSwapchainFrame(
    const SwapchainHandle swapchain_handle) {
  if (GetSwapchainValid(swapchain_handle)) {
...
#if defined BGF_DISPLAY_MANAGER_GLES
bool DisplayManager::GetGraphicsAPIResourcesGLES(
...
bool DisplayManager::GetSwapchainFrameResourcesGLES(
  ...
  if (frame_handle != kInvalid_swapchain_handle) {
    if (active_api_ == kGraphicsAPI_GLES) {
      if (api_gles_->GetAPIStatus() == kGraphicsAPI_Active) {
        return api_gles_->GetSwapchainFrameResourcesGLES(frame_handle,
                                                         frame_resources);
      }
    }
  }
...
#if defined BGF_DISPLAY_MANAGER_VULKAN
bool DisplayManager::GetGraphicsAPIResourcesVk(
...
bool DisplayManager::GetSwapchainFrameResourcesVk(
    const SwapchainFrameHandle frame_handle,
    SwapchainFrameResourcesVk& frame_resources, bool acquire_frame_image) {
  if (frame_handle != kInvalid_swapchain_handle) {
    if (active_api_ == kGraphicsAPI_Vulkan) {
      if (api_vulkan_->GetAPIStatus() == kGraphicsAPI_Active) {
        return api_vulkan_->GetSwapchainFrameResourcesVk(
            frame_handle, frame_resources, acquire_frame_image);
      }
    }
  }
  return false;
...
bool DisplayManager::SetSwapchainChangedCallback(
    SwapchainChangedCallback callback, void* user_data) {
  if (active_api_ != kGraphicsAPI_None &&
      api_->GetAPIStatus() == kGraphicsAPI_Active) {
